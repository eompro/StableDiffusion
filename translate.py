...
class KoreanStableDiffusion(DiffusionPipeline):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion in Korean languages.
    ...
    def __init__(
        ...
        # if hasattr(scheduler.config, "steps_offset") and scheduler.config.steps_offset != 1: ## steps_offset 속성 없다고함 
        ...
        **kwargs,
        ...
        if do_classifier_free_guidance:
            ...
            if negative_prompt is None:
                ...
                negative_prompt_language = "hi" # 한국어 하드코딩
                ###############################################################
                ...
                if isinstance(negative_prompt, list):
                    ...
                    negative_prompt_languages = 'hi' # 한국어 하드코딩
                    ########################################################################
                    for index in range(batch_size):
        ...
        if latents is None:
            if self.device.type == "mps":
                ...
                latents = torch.randn(latents_shape, generator=generator, device=self.device, dtype=latents_dtype).to(
                    self.device
                )
            else:
                latents = torch.randn(latents_shape, generator=generator, device=self.device, dtype=latents_dtype).to(self.device)
        else:
